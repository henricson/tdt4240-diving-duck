...
class ScoreApi(basePath: kotlin.String = "/") : ApiClient(basePath) {
     ...
     * 
     * 
     * @return kotlin.Array<ScoreResponse>
     */
    @Suppress("UNCHECKED_CAST")
    fun apiScoreGet(): kotlin.Array<ScoreResponse> {
        val localVariableConfig = RequestConfig(
                RequestMethod.GET,
                "/api/Score"
        )
        val response = request<kotlin.Array<ScoreResponse>>(
                localVariableConfig
        )

        return when (response.responseType) {
            ResponseType.Success -> (response as Success<*>).data as kotlin.Array<ScoreResponse>
     ...
     */
    @Suppress("UNCHECKED_CAST")
    fun apiScorePost(body: ScoreDTO? = null): ScoreResponse {
        val localVariableBody: kotlin.Any? = body
        val localVariableConfig = RequestConfig(
                RequestMethod.POST,
                "/api/Score"
        )
        val response = request<ScoreResponse>(
                localVariableConfig, localVariableBody
        )

        return when (response.responseType) {
            ResponseType.Success -> (response as Success<*>).data as ScoreResponse
            ResponseType.Informational -> TODO()
            ResponseType.Redirection -> TODO()
            ResponseType.ClientError -> throw ClientException((response as ClientError<*>).body as? String ?: "Client error")
            ResponseType.ServerError -> throw ServerException((response as ServerError<*>).message ?: "Server error")
